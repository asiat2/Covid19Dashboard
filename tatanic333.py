...
print(df_tatanic.iloc[:,:].describe())
pd.crosstab(df_tatanic['Sex'], df_tatanic['Survived']).plot(kind='bar')
pd.crosstab(df_tatanic['Pclass'], df_tatanic['Survived']).plot(kind='bar')
pd.crosstab(df_tatanic['Embarked'], df_tatanic['Survived']).plot(kind='bar')
pd.crosstab(df_tatanic['SibSp'], df_tatanic['Survived']).plot(kind='bar')
plt.show()

sns.heatmap(pd.crosstab([df_tatanic.Survived, df_tatanic.Sex], [df_tatanic.Pclass]),
            cmap="YlGnBu", annot=True, cbar=False)   
# ================ Data preprocessing =============================================
df_tatanic_final = final_process(df_tatanic)  
df_tatanic_final.info()
df_tatanic_final=df_tatanic_final.drop(['Cabin'],axis=1)
...
df_tatanic_final['Age_group'] = df_tatanic['Age_group'].astype('category')
df_tatanic['Pclass'] = df_tatanic['Pclass'].astype('category')
df_tatanic['Survived'] = df_tatanic['Survived'].astype('category')
...
df_processed['Pclass'] = df_processed['Pclass'].replace({'First':1,'Second':2,'Third':3})
